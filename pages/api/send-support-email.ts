...
export default async function handler(
  ...
  const { ticketId, userEmail, subject, category, priority, description, userId } = req.body;
  ...
  if (!process.env.RESEND_API_KEY) {
    ...
    return res.status(200).json({ message: 'Email notification skipped (not configured)' });
  }
